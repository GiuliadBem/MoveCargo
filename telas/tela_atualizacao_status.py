...
class TelaAtualizacaoStatus:
    ...
    def __criar_layout_sem_fretes(self) -> list:
        """Cria o layout para quando não há fretes para mostrar."""
        return [[sg.Text("⚠️ Nenhum frete encontrado.", text_color="red", font=("Arial", 12))]]

    def __criar_layout_lista_fretes(self, lista_fretes: list[dict], perfil: str) -> list:
        """Cria o layout para a lista de fretes."""
        layout = []
    ...
    def mostrar_fretes_para_atualizacao(self, lista_fretes: list[dict], perfil: str = "gerente"):
        ...
        if not lista_fretes:
            layout.extend(self.__criar_layout_sem_fretes())
        else:
            layout.extend(self.__criar_layout_lista_fretes(lista_fretes, perfil))
